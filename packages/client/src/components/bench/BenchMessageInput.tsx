...
export function BenchMessageInput({ onSend, onTyping, disabled, placeholder, onlineUsers = [] }: BenchMessageInputProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="border-t border-dark-200 bg-dark-50 px-4 py-4 relative">
      {/* @mention autocomplete dropdown */}
      {mentionResults.length > 0 && mentionQuery !== null && (
        <div className="absolute bottom-full left-4 right-4 mb-1 bg-dark-100 rounded-lg shadow-lg border border-dark-200 py-1 z-20 max-h-48 overflow-y-auto">
          {mentionResults.map((u, i) => (
            <button
              ...
              className={`w-full flex items-center gap-2 px-3 py-2 text-left text-sm transition-colors ${
                i === mentionIndex ? "bg-gold/15 text-gold" : "text-[#E5E5E5] hover:bg-dark-200"
              }`}
            >
              {u.userPhotoPath ? (
                <img src={u.userPhotoPath} alt={u.userName} className="size-6 rounded-full object-cover" />
              ) : (
                <div className="size-6 rounded-full bg-gold/15 flex items-center justify-center">
                  <span className="text-xs font-semibold text-gold">{u.userName.charAt(0).toUpperCase()}</span>
                </div>
              )}
      ...
      <div className="flex items-center gap-3">
        <input
          ...
          className="flex-1 px-4 py-3.5 text-base border border-dark-200 rounded-xl bg-dark text-white placeholder-[#555555] focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold/50 disabled:opacity-50 disabled:cursor-not-allowed"
          autoComplete="off"
        />
        <button
          type="submit"
          disabled={!text.trim() || disabled}
          className="p-3 rounded-xl bg-gold text-dark hover:bg-gold-hover disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
        >
