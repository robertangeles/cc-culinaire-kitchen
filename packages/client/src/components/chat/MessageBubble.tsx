    ...
    <div className={`flex gap-3 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
      ...
      <div
        className={`rounded-2xl px-4 py-3 ${
          isUser
            ? "bg-gold/10 border border-gold/20 text-[#E5E5E5]"
            : "bg-dark-50 border border-dark-200 text-[#E5E5E5]"
        }`}
      >
        {isUser ? (
          <p className="whitespace-pre-wrap leading-relaxed">{textContent}</p>
        ) : (
          <div className="prose prose-invert prose-sm max-w-none prose-p:my-1 prose-p:text-[#E5E5E5] prose-p:leading-relaxed prose-ul:my-1 prose-ol:my-1 prose-li:my-0.5 prose-li:text-[#E5E5E5] prose-headings:mt-3 prose-headings:mb-1 prose-headings:text-[#FAFAFA] prose-strong:text-[#FAFAFA] prose-code:bg-dark prose-code:border prose-code:border-dark-200 prose-code:rounded-md prose-code:px-1.5 prose-code:py-0.5 prose-code:text-gold prose-pre:bg-dark prose-pre:border prose-pre:border-dark-200 prose-pre:rounded-xl prose-a:text-gold prose-a:no-underline hover:prose-a:underline">
            <ReactMarkdown
