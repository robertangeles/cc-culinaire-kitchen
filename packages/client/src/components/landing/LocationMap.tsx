...
export function LocationMap() {
  return (
    <motion.div
      ...
      className="w-full max-w-[320px] rounded-2xl border border-cocoa-200 bg-cocoa-100 p-5 shadow-2xl shadow-black/30"
    >
      {/* Header */}
      <p className="mb-4 text-[10px] font-semibold uppercase tracking-widest text-cocoa-500">
      ...
      >
        {locations.map((loc, i) => (
          <motion.div
            ...
            className={`relative flex items-center gap-3 rounded-xl border ${loc.borderColor} bg-cocoa-50 px-3 py-3 ${loc.indented ? "ml-6" : ""}`}
          >
            {/* Connector line for indented items */}
            {loc.indented && (
              <div className="absolute -left-4 top-1/2 h-px w-4 bg-cocoa-200" />
            )}
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-[10px] text-cocoa-500">
      ...
      <div
        className="pointer-events-none absolute left-[2.85rem] top-[4.5rem] -z-10 hidden h-[calc(100%-6rem)] w-px bg-cocoa-200"
        aria-hidden
      />
    </motion.div>
  );
