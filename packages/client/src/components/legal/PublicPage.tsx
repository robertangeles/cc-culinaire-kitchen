...
export function PublicPage({ slug: slugProp, surface }: Props) {
  ...
  return (
    <div className="min-h-screen bg-dark text-[#E5E5E5] flex flex-col">
      {/* Slim header */}
      <header className="border-b border-[#1A1A1A] bg-dark">
        <div className="max-w-3xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link to="/" className="text-gold hover:text-[#E5C594] text-sm font-medium transition-colors">
            ← CulinAIre Kitchen
          </Link>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-3xl w-full mx-auto px-6 py-12">
        {state.status === "loading" && (
          <div className="flex items-center justify-center text-dark-500 py-24">
        ...
        {state.status === "not-found" && (
          <div className="text-center py-24">
            <FileQuestion className="size-10 mx-auto mb-3 text-gold/40" />
            <h1 className="text-2xl font-bold text-white mb-2">Page not found</h1>
            <p className="text-dark-600">This page doesn't exist or hasn't been published yet.</p>
            <Link to="/" className="inline-block mt-6 text-gold hover:text-[#E5C594] text-sm transition-colors">
              ← Back home
            </Link>
          </div>
        )}
        ...
        {state.status === "ok" && (
          <article>
            ...
            {state.page.bodyMd.trim() ? (
              <div className="prose prose-invert prose-base max-w-none prose-headings:text-[#FAFAFA] prose-p:text-[#E5E5E5] prose-strong:text-[#FAFAFA] prose-a:text-gold prose-a:no-underline hover:prose-a:underline prose-li:text-[#E5E5E5] prose-code:text-gold prose-code:bg-dark-50 prose-code:border prose-code:border-dark-200 prose-code:rounded prose-code:px-1 prose-code:py-0.5 prose-pre:bg-dark-50 prose-pre:border prose-pre:border-dark-200">
                <ReactMarkdown remarkPlugins={[remarkGfm]}>{state.page.bodyMd}</ReactMarkdown>
              </div>
            ) : (
              <p className="text-dark-500 italic">This page has no content yet.</p>
            )}
          </article>
        )}
      </main>
