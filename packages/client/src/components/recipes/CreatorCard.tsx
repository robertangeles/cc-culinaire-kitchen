...
export function CreatorCard({ creator }: CreatorCardProps) {
  ...
  return (
    ...
    >
      <span className="text-xs text-dark-500">Added by</span>
      <span className={`text-xs font-medium text-dark-600 ${hasPopoverContent ? "cursor-pointer hover:text-gold transition-colors" : ""}`}>
        {displayName}
      </span>
      ...
      {showPopover && hasPopoverContent && (
        <div
          className="absolute bottom-full left-0 mb-2 w-64 bg-dark-50 rounded-xl shadow-lg border border-dark-200 p-4 z-50"
          onMouseEnter={handleMouseEnter}
          onMouseLeave={handleMouseLeave}
        >
          {/* Arrow */}
          <div className="absolute -bottom-1.5 left-6 w-3 h-3 bg-dark-50 border-r border-b border-dark-200 rotate-45" />
          ...
          <div className="flex items-start gap-3">
            ...
            {creator.userPhotoPath ? (
              ...
              <div className="size-12 rounded-full bg-dark-100 flex items-center justify-center flex-shrink-0">
                <User className="size-6 text-dark-600" />
              </div>
            )}
            ...
            <div className="min-w-0 flex-1">
              ...
              {creator.restaurantName && (
                <p className="text-xs text-gold truncate">{creator.restaurantName}</p>
              )}
            </div>
          </div>
          ...
          {/* Bio */}
          {creator.userBio && (
            <p className="text-xs text-dark-600 mt-2 line-clamp-3 leading-relaxed">
              {creator.userBio}
            </p>
          )}
          ...
          {/* Social links */}
          {socials.length > 0 && (
            <div className="flex items-center gap-2 mt-3 pt-2 border-t border-dark-200">
              {socials.map(({ key, url }) => {
                ...
                return (
                  <a
                    ...
                    className={`text-dark-500 ${social.color} transition-colors`}
                    title={social.label}
