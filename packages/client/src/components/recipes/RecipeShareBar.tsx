...
export function RecipeShareBar({
  ...
  const btnClass = "p-2 rounded-lg bg-dark-100 hover:bg-dark-200 text-dark-600 hover:text-white transition-colors group relative";
  ...
  return (
    <div className="flex items-center gap-1 flex-wrap">
      ...
      {typeof navigator !== "undefined" && typeof navigator.share === "function" && (
        <button onClick={handleNativeShare} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gold text-dark text-xs font-medium hover:bg-gold-hover transition-colors" title="Share to any app">
      ...
      <button onClick={handleInstagram} className={btnClass} title="Copy caption for Instagram">
        ...
        {false && (
          <span className="absolute -top-8 left-1/2 -translate-x-1/2 px-2 py-1 text-xs text-white bg-dark-100 rounded whitespace-nowrap">
            Caption copied!
          </span>
        )}
      </button>
      ...
      <button onClick={() => setEmailOpen(true)} className={btnClass} title="Email recipe">
        <Mail className={`${iconClass} text-dark-600`} />
      </button>
      ...
      <button onClick={handleCopyLink} className={btnClass} title="Copy link">
        {copied === "link" ? (
          ...
          <Link2 className={`${iconClass} text-dark-600`} />
        )}
        {copied === "link" && (
          <span className="absolute -top-8 left-1/2 -translate-x-1/2 px-2 py-1 text-xs text-white bg-dark-100 rounded whitespace-nowrap">
            Link copied!
      ...
      {igOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-dark-50 border border-dark-200 rounded-xl shadow-xl w-full max-w-sm mx-4 p-5">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-[#FAFAFA]">Share on Instagram</h3>
              <button onClick={() => setIgOpen(false)} className="p-1 rounded hover:bg-dark-200">
                <XIcon className="size-4 text-dark-600" />
              </button>
            </div>
            ...
            <div className="flex gap-2">
              <button onClick={() => setIgOpen(false)} className="flex-1 py-2 text-sm font-medium text-dark-600 bg-dark-100 rounded-lg hover:bg-dark-200 transition-colors">
                Done
              </button>
              <a
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-dark-50 border border-dark-200 rounded-xl shadow-xl w-full max-w-sm mx-4 p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setEmailOpen(false)} className="p-1 rounded hover:bg-dark-200">
                <XIcon className="size-4 text-dark-600" />
              </button>
            </div>
            <input
              ...
              className="w-full px-4 py-3 border border-dark-200 bg-dark rounded-xl text-sm text-white placeholder-dark-400 focus:ring-2 focus:ring-gold/50 focus:border-gold mb-3"
            ...
            <div className="flex gap-2">
              <button onClick={() => setEmailOpen(false)} className="flex-1 py-2 text-sm font-medium text-dark-600 bg-dark-100 rounded-lg hover:bg-dark-200 transition-colors">
                Cancel
              </button>
              <button
                onClick={handleEmailSend}
                disabled={emailSending || !emailTo.trim()}
                className="flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium text-dark bg-gold rounded-lg hover:bg-gold-hover disabled:opacity-50 transition-colors"
              >
