...
export function PromptsTab() {
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="w-64 flex-shrink-0 border-r border-stone-200 flex flex-col">
  ...
  return (
    <button
      ...
      className={`w-full text-left px-4 py-2.5 transition-colors ${
        isActive
          ? "bg-amber-50 border-r-2 border-amber-600 text-stone-900"
          : "text-stone-600 hover:bg-stone-50 hover:text-stone-800"
      }`}
    >
      <span className="block text-sm font-medium truncate">{prompt.promptName}</span>
      {prompt.promptKey && (
        <span className="block text-xs text-stone-400 truncate">{prompt.promptKey}</span>
      )}
    </button>
  );
}
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 px-8 py-4 min-h-0">
        <textarea
          ...
          className="w-full h-full resize-none rounded-lg border border-stone-300 bg-white px-4 py-3 font-mono text-sm text-stone-800 leading-relaxed focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent"
      ...
      {/* Bottom bar */}
      <div className="px-8 py-4 border-t border-stone-200 bg-stone-50 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button
            onClick={reset}
            disabled={isSaving}
            className="flex items-center gap-2 px-4 py-2 text-sm text-stone-600 bg-white border border-stone-300 rounded-lg hover:bg-stone-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-8 py-6 border-b border-stone-200 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-stone-900">New Prompt</h1>
          <p className="mt-1 text-sm text-stone-500">
      ...
      <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0">
          ...
          <div>
            ...
            {previewKey && (
              <p className="mt-1 text-xs text-stone-400">
                Key: <code className="bg-stone-100 px-1 rounded">{previewKey}</code>
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 px-8 pb-4 min-h-0">
          ...
          <textarea
            ...
            className="w-full h-[calc(100%-24px)] resize-none rounded-lg border border-stone-300 bg-white px-4 py-3 font-mono text-sm text-stone-800 leading-relaxed focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent"
            spellCheck={false}
          />
        </div>
