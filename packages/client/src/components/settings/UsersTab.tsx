...
export function UsersTab() {
  ...
  const statusBadge = (status: string) => {
    ...
    return (
      <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${colors[status] ?? "bg-dark-100 text-[#E5E5E5]"}`}>
        {status}
      </span>
    );
  };
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Sub-tab bar */}
      <div className="flex gap-1 px-6 pt-5 pb-0 border-b border-dark-200">
        <button
          onClick={() => setSubTab("users")}
          className={`px-4 py-2 text-sm font-medium rounded-t-lg border-b-2 transition-colors ${
            subTab === "users"
              ? "border-gold text-gold"
              : "border-transparent text-dark-600 hover:text-[#E5E5E5]"
          }`}
        >
          User Management
        </button>
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-dark-600" />
          <input
            type="text"
            value={search}
            onChange={(e) => { setSearch(e.target.value); setPage(1); }}
            placeholder="Search users..."
            className="pl-9 pr-3 py-1.5 text-sm border border-dark-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold/50"
          />
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-dark-200">
                {users.map((u) => (
                  <tr
                    key={u.userId}
                    className="hover:bg-dark cursor-pointer"
                    ...
                    <td className="py-3">
                      <div className="flex items-center gap-2">
                        {u.userPhotoPath ? (
                          ...
                          <div className="size-7 rounded-full bg-gold/15 flex items-center justify-center text-xs font-medium text-gold">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-dark-600">{u.userEmail}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-3" onClick={(e) => e.stopPropagation()}>
                      <div className="flex flex-wrap gap-1">
                        {u.roles.map((r) => (
                          <span
                            key={r}
                            className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs bg-dark-100 text-[#E5E5E5] cursor-pointer hover:bg-red-900/40 hover:text-red-400 transition-colors"
                        ...
                        <select
                          className="text-xs border border-dark-200 rounded px-1 py-0.5 text-dark-600"
                          value=""
                          onChange={(e) => {
                      ...
                      {editingSessions === u.userId ? (
                        <div className="flex items-center gap-1">
                          <input
                            ...
                            className="w-16 text-sm border border-dark-200 rounded px-1 py-0.5"
                          />
                          <button
                            onClick={() => handleSaveSessions(u.userId)}
                            className="text-xs text-gold hover:text-gold font-medium"
                          >
                            Save
                          </button>
                        </div>
                        ...
                        <button
                          ...
                          className="text-[#E5E5E5] hover:text-gold transition-colors"
                        >
                          {u.freeSessions}
                        </button>
                      )}
