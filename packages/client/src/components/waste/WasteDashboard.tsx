...
export function WasteDashboard({ onSwitchTab, teamView = false }: Props) {
  ...
  const trendColor = trendDirection === "up" ? "text-red-400" : trendDirection === "down" ? "text-emerald-400" : "text-dark-600";
    ...
    <div>
      ...
      {period === "custom" && (
        <div className="flex flex-wrap gap-3 mb-6">
          <input
            ...
            className="px-3 py-2 bg-dark-50 border border-dark-200 rounded-lg text-white text-sm focus:ring-2 focus:ring-gold/50 min-h-[44px]"
          />
      ...
      {!loading && !error && (!data || (Number(data?.totalEntries ?? 0) === 0)) && (
        <div className="text-center py-16">
          <Leaf className="size-16 mx-auto text-dark-500 mb-4" />
      ...
      {!loading && !error && data && Number(data?.totalEntries ?? 0) > 0 && (
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <SummaryCard
              icon={<Scale className="size-5 text-gold" />}
              label="Total Waste"
            ...
            />
            <SummaryCard
              icon={<ClipboardList className="size-5 text-gold" />}
              label="Entries"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-dark-50 rounded-xl p-6 border border-dark-200">
              <h3 className="text-sm font-medium text-dark-600 uppercase tracking-wider mb-4">Top Ingredients by Cost</h3>
            ...
            {/* Top 5 by weight */}
            <div className="bg-dark-50 rounded-xl p-6 border border-dark-200">
              <h3 className="text-sm font-medium text-dark-600 uppercase tracking-wider mb-4">Top Ingredients by Weight</h3>
          ...
          {/* Waste by reason */}
          <div className="bg-dark-50 rounded-xl p-6 border border-dark-200 mb-8">
            <h3 className="text-sm font-medium text-dark-600 uppercase tracking-wider mb-4">Waste by Reason</h3>
            {!data?.byReason?.length ? (
              ...
              <div className="space-y-3">
                {data.byReason.map((r) => {
                  ...
                  const barColor = REASON_COLORS[r?.reason ?? ""] ?? "bg-dark-200";
          ...
          <div className="bg-dark-50 rounded-xl p-6 border border-dark-200 mb-8">
            <h3 className="text-sm font-medium text-dark-600 uppercase tracking-wider mb-4">Daily Waste Trend</h3>
          ...
          {/* Industry benchmark */}
          <div className="bg-dark-50/50 rounded-xl p-4 border border-dark-200 flex items-start gap-3">
            <BarChart3 className="size-5 text-gold shrink-0 mt-0.5" />
            <div>
  ...
  return (
    <div className="bg-dark-50 rounded-xl p-4 border border-dark-200">
      <div className="flex items-center gap-2 mb-2">
        {icon}
        <span className="text-xs text-dark-600 font-medium uppercase tracking-wider">{label}</span>
      </div>
      ...
      {items.map((item) => (
        <div key={item?.label ?? "unknown"}>
          <div className="flex items-center justify-between text-sm mb-1">
            ...
            <span className="text-dark-600 shrink-0">{item?.display}</span>
          </div>
          <div className="w-full bg-dark-100 rounded-full h-2">
            <div
              className={`h-2 rounded-full transition-all duration-500 ${item?.color || color || "bg-gold/100"}`}
      ...
      {data.map((d) => {
        ...
        return (
          <div key={d?.date} className="flex flex-col items-center flex-1 min-w-[24px] group relative">
            {/* Tooltip */}
            <div className="hidden group-hover:block absolute bottom-full mb-1 bg-dark-100 text-xs text-white px-2 py-1 rounded shadow-lg whitespace-nowrap z-10">
              {dateLabel}: {weight} kg — ${cost.toFixed(2)}
            </div>
            <div
              className="w-full bg-gold/100 rounded-t transition-all duration-300 hover:bg-gold-hover"
              style={{ height: `${heightPct}%` }}
            />
            <span className="text-[9px] text-dark-500 mt-1 truncate w-full text-center">
