...
const TYPE_BADGE_STYLES: Record<string, string> = {
  recipe: "bg-gold/20 text-gold border-gold/30",
    ...
    <div>
      ...
      <div className="text-center mb-8">
        <button
          ...
          disabled={loading}
          className="inline-flex items-center gap-2 px-6 py-3 bg-gold hover:bg-gold-hover disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium rounded-lg transition-colors min-h-[48px]"
        >
          {loading ? (
            <>
      ...
      {loading && (
        <div className="text-center py-12">
          <Loader2 className="size-10 mx-auto text-gold animate-spin mb-4" />
          <p className="text-[#E5E5E5] font-medium">Our AI chef is thinking of ways to reduce your waste...</p>
          <p className="text-xs text-dark-500 mt-1">This usually takes a few seconds.</p>
        </div>
      )}
      ...
      {hasGenerated && !loading && suggestions.length === 0 && (
        <div className="text-center py-12">
          <Trash2 className="size-10 mx-auto text-dark-500 mb-3" />
          <p className="text-dark-600 font-medium">No suggestions yet</p>
      ...
      {!hasGenerated && !loading && (
        <div className="text-center py-12">
          <Lightbulb className="size-10 mx-auto text-dark-500 mb-3" />
          <p className="text-dark-600 font-medium">Ready for suggestions?</p>
          <p className="text-sm text-dark-500 mt-1">
        ...
        <div className="space-y-4">
          {suggestions.map((s) => {
            ...
            return (
              <div
                key={s?.id}
                className="bg-dark-50 rounded-xl p-5 border border-dark-200"
              >
                {/* Header row */}
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-4 py-2 bg-gold/20 hover:bg-gold/30 text-gold text-sm font-medium rounded-lg transition-colors border border-gold/30 min-h-[44px]"
                  >
                  ...
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-lg transition-colors border min-h-[44px] ${
                      isSaved
                        ...
                        : "bg-dark-100/50 text-dark-600 border-dark-200 hover:bg-dark-100 hover:text-[#E5E5E5]"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-4 py-2 bg-dark-100/50 hover:bg-dark-100 text-dark-500 hover:text-[#E5E5E5] text-sm font-medium rounded-lg transition-colors border border-dark-200 min-h-[44px]"
                  >
                    Dismiss
                  </button>
                </div>
