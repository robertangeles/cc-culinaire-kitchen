...
export async function handleCreatePrompt(
  ...
  try {
    ...
    log.info(
      { promptName: result.promptName, promptKey: result.promptKey },
      "Prompt created",
    );
...
export async function getPrompt(
  ...
  try {
    ...
    const { content, modelId, promptKey } = await getPromptRawForAdmin(name);
    log.info({ name }, "Prompt retrieved");
    res.json({ name, content, modelId, promptKey });
  } catch (err) {
