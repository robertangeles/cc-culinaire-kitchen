...
describe("rateLimiter — chatRateLimit config", () => {
  ...
  it("constructs with 20 requests per 60 seconds", async () => {
    ...
    // Eight limiters are constructed: chat + mobile prompt + mobile RAG +
    // mobile feedback + auth + compliance document view + compliance
    // document edit + compliance report. Assert call count first so we
    // catch any future limiter being added or removed silently.
...
describe("rateLimiter — complianceDocumentEditRateLimit config", () => {
  beforeEach(() => {
    mockRateLimit.mockClear();
    vi.resetModules();
  });
  ...
  it("keys per user, so one caller cannot spend another's budget", async () => {
    ...
    // deleteDocument holds a pooled DB connection across a Cloudinary call —
    // a per-user key stops one caller pinning the shared pool by hammering
    // edit/delete on their own document.
    expect(config.keyGenerator({ user: { sub: 7 }, ip: "203.0.113.7" })).toBe(
      "compliance-edit-user-7",
    );
    expect(config.keyGenerator({ user: { sub: 8 }, ip: "203.0.113.7" })).toBe(
      "compliance-edit-user-8",
    );
  });
...
describe("rateLimiter — complianceReportRateLimit config", () => {
  ...
  it("constructs with 10 requests per 5 minutes", async () => {
    ...
    const config = mockRateLimit.mock.calls[7][0] as Record<string, unknown>;
  ...
  it("keys per user, so one caller cannot spend another's budget", async () => {
    ...
    const config = mockRateLimit.mock.calls[7][0] as {
      keyGenerator: (req: { user?: { sub: number }; ip?: string }) => string;
    };
