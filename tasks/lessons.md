...
- **Fix (not yet applied — see todo.md)**: Make `authService` read secrets at call-time, per lesson #3's rule — e.g. `function accessSecret() { return process.env.JWT_ACCESS_SECRET ?? "dev-access-secret"; }` used inside `generateTokens`/`verifyAccessToken`/MFA — OR introduce a single `src/bootstrap/env.ts` that runs `config()` + `applyEnvPrefix()` at its own module top-level and is imported first by every entry point and script. Caveat: applying the fix means dev tokens start signing with the real `DEV_JWT_ACCESS_SECRET`, so all current dev sessions (signed with the old fallback) are invalidated once — users re-login. Prod unchanged.
