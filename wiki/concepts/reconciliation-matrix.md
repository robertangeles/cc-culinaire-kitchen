...
| **Area movement** (`stockMovementService.createMovement`) | **NONE — zero stock effect by design.** Stock Room → Bar keeps the item on site and sellable; `stock_level` is not read or written. The service does not even import it. | None. No FIFO layer moves, no WAC recompute — the item never changed hands or location, only shelf. | Nothing. This is the point: a move must balance against NOTHING because nothing changed. The invariant is asserted directly — an integration test compares the whole `stock_level` row before/after, so even a version bump fails. Before this existed, the only vocabulary for "restocked the bar" was to CONSUME it, which deducted at the move AND again at the sale, surfacing as phantom yield variance. | none |
