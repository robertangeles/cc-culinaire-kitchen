...
- Verified: server 544/544 (route auth matrix for the 3 PATCH routes + real-DB integration for pin/correct→re-embed/scope-toggle incl. the org-admin boundary and colleague-visibility), client 58/58 (MemoryRow actions, ScopeToggle, hook mutations), tsc + build green, local migration idempotent (twice), and a live HTTP PATCH smoke where the DB reflected pin=true/scope=org/body edited and the running worker re-embedded the corrected body against real OpenRouter.
- Deploy prereq: run `addBrainPinColumn.ts` on prod (backup-first) before deploy — reconnects to the parked central-backup-repo topic. Not yet committed. Next: T14c (org-admin management surface).
